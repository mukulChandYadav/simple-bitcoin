defmodule SB.Block do
  ...
  defstruct block_header: %SB.BlockHeader{}, timestamp: :os.system_time(:millisecond), target: 4, tx: [], block_id: 0, prev_block: nil
  ...
  defp prepare_block_header(prev_block, nonce, txs) do
    ...
    tx_hash = Enum.reduce(txs, SB.CryptoHandle.hash("initial", :sha256), fn x, acc -> SB.CryptoHandle.hash(inspect(acc <> inspect(x)), :sha256) end)
  ...
  def prepare_block(base_block, nonce, new_tx) do
    ...
    prev_block = if(base_block.block_id == 0) do
      nil
    else
      base_block
    end
